...
export default async function MotherDaughterClassesPage() {
      ...
      <div className="max-w-2xl mx-auto px-4 py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-[#7a6f68] font-light text-sm">
            A shared experience to learn professional make-up techniques together. Choose an upcoming session below.
          </p>
        </div>
        ...
        {sessions.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#e8e0d8] p-10 text-center">
            ...
            <p className="text-sm text-[#7a6f68] font-light">
              No Mother &amp; Daughter classes are currently scheduled. Please check back soon or{" "}
              <a
                href="mailto:amanda@cocoonskinandbeauty.com.au"
                className="text-[#044e77] hover:underline"
              >
                contact Amanda
              </a>{" "}
          ...
          <div className="flex flex-col gap-4">
            {sessions.map((session) => {
              ...
              return (
                <Link
                  key={session.id}
                  href={full ? "#" : `/book/mother-daughter-classes/${session.id}`}
                  aria-disabled={full}
                  className={[
