...
/**
 * Validates a coupon code server-side for a class ticket booking.
 * Class bookings aren't categorised the same way appointments are, so
 * category-restricted coupons are rejected here rather than matched —
 * only coupons with applies_to = 'all' are valid for class bookings.
 * @param code        The coupon code entered by the user.
 * @param amountCents The pre-discount amount so we can compute the discount.
 */
export async function validateCouponForClass(
  code: string,
  amountCents: number,
): Promise<CouponValidation> {
  const normalised = code.trim().toUpperCase();
  ...
  // Category-restricted coupons don't apply to class bookings
  if (coupon.applies_to === "specific_categories") {
    return { valid: false, error: "This discount code is not valid for class bookings." };
  }
